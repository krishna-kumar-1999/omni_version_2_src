...
var port1 = null;
var port2 = null;
if(localStorage.getItem('server_FQDN') == 'localhost'){
	//   webrtcserver = window.location.hostname;
	  port1 = 4189;	
	  port2 = 4089;	

	 webrtcserver ='c4cteams.my3cx.sg';
	//  webrtcserver ='nextcomm.my3cx.com.br';
}else{
	 webrtcserver = localStorage.getItem('server_FQDN');
	 port1 = 8189;	
	 port2 = 8089;	
}
...
if(window.location.protocol === 'http:')
	...
	server = [
		"wss://"+webrtcserver+":"+port1,
		"https://"+webrtcserver+":"+port2+"/janus"
	];

else
	// server = [
	// 	"wss://"+webrtcserver+":4189",
	// 	"https://"+webrtcserver+":4089/janus"
	// ];
						...
						janus.attach(
							{
								...
								onmessage: function(msg, jsep) {
									...
									if(result && result["event"]) {
										...
										//alert(pbxSettingsStatusV2);
											...
											var audio = document.getElementById('incommingCalltone');
											audio.currentTime = 0;
										...
										clearTimeout(callduration_timer);
											...
											localStorage.setItem('call_end_status','1');
								...
								onlocalstream: function(stream) {
									...
									if($('#myvideo').length === 0)
										$('#videoleft').append('<video class="rounded centered 1" id="myvideo" width="100%" height="100%" autoplay playsinline muted="muted"/>');
									...
									if(sipcall.webrtcStuff.pc.iceConnectionState !== "completed" &&
										...
										// No remote video yet
										$('#videoright').append('<video class="rounded centered 2" id="waitingvideo" width="100%" height="100%" />');
										if(spinner == null) {
								...
								onremotestream: function(stream) {
									...
									if($('#remotevideo').length === 0) {
										$('#videoright').parent().find('h3').html(
											...
											'</span>');
										$('#videoright').append(
											'<video class="rounded centered 3 hide" id="remotevideo" width="100%" height="100%" autoplay playsinline/>');
										for(var i=0; i<12; i++) {
											if(i<10)
										...
										$('#info').click(function() {
											bootbox.dialog({
												message: 'Type: <input class="form-control" type="text" id="type" >' +
													'<br/>Content: <input class="form-control" type="text" id="content" >',
...
function getinStatusV2() {
	localStorage.setItem('dialer_register', pbxSettingsStatusv2);
...
function doCall(ev) {
	...
	var user_port = $('#sip_port').val();
	$('#outcall_number').val(username);
	if(user_port=="5060"){
		username = "sip:"+username+"@"+sip_urld;
	}else{
		username = "sip:"+username+"@"+sip_urld+":"+user_port;
	}
...
function doCall3(ev,username) {
    var sip_urld = $('#sip_urld').val();
	$('#outcall_number').val(username);
	var sip_ports = $('#sip_port').val();
	if(sip_ports=="5060"){
	username = "sip:"+username+"@"+sip_urld;
	}else{
	username = "sip:"+username+"@"+sip_urld+":"+sip_ports;
	}
	 console.log(username);
    $('#peer').val(username);
	// Call someone (from the main session or one of the helpers)
	...
	janus.attach(
		{
			...
			onlocalstream: function(stream) {
				...
				if($('#myvideo' + helperId).length === 0)
					$('#videoleft' + helperId).append('<video class="rounded centered 4" id="myvideo' + helperId + '" width="100%" height="100%" autoplay playsinline muted="muted"/>');
				...
				if(helpers[helperId].sipcall.webrtcStuff.pc.iceConnectionState !== "completed" &&
					...
					// No remote video yet
					$('#videoright' + helperId).append('<video class="rounded centered 5" id="waitingvideo' + helperId + '" width="100%" height="100%" />');
			...
			onremotestream: function(stream) {
				...
				if($('#remotevideo' + helperId).length === 0) {
					...
					$('#videoright' + helperId).append(
						'<video class="rounded centered 6 hide" id="remotevideo' + helperId + '" width="100%" height="100%" autoplay playsinline/>');
					for(var i=0; i<12; i++) {
						if(i<10)
					...
					$('#transfer' + helperId).click(function() {
						bootbox.dialog({
							message: '<input class="form-control" type="text" id="transferto" >',
							title: "Insert the address to transfer the call to",
							buttons: {
								cancel: {
									label: "Cancel",
...
function makecallTransfer2(username){
	...
	var sip_urld = $('#sip_urld').val();
	// $('#outcall_number').val(username);
	// username = "sip:998@"+sip_urld+"1090";
	// username = "sip:"+username+"@"+sip_urld;
	var sip_ports = $('#sip_port').val();
	if(sip_ports=="5060"){
	username = "sip:"+username+"@"+sip_urld;
...
function hookStatus(status){
	console.log(status);
	if(status=="Ready For Calls"){
		hookStatuss = status;
		$('#status_indi_bar').attr("class", "status-indication-bar on-hook-status");
	} else if(status=="Server Down" || status==="Disconnected"){
		hookStatuss = status;
		...
		if(hookStatuss=="Ready For Calls"){
			hookStatuss = 'Ready For Calls';
			$('#status_indi_bar').attr("class", "status-indication-bar on-hook-status");
		} else {
			hookStatuss = 'Not Connected';
			$('#status_indi_bar').attr("class", "status-indication-bar dnd-status");
		}
	}
	$('#hook_statuss').html(hookStatuss);
}
